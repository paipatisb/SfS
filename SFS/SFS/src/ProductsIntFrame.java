...
public class ProductsIntFrame extends C_P_InternalFrame implements ActionListener ,MouseListener  {
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource().equals(this.btnCreateNewRecord)){
			...
			if(dialogueResult==JOptionPane.YES_OPTION){
				try{
					Product prod=new Product(txtFields.get(0).getText().toUpperCase(),txtFields.get(1).getText().toUpperCase(),
							txtFields.get(2).getText().toUpperCase(),txtFields.get(3).getText().toUpperCase(),
							txtFields.get(4).getText().toUpperCase(),txtFields.get(5).getText().toUpperCase(),
							txtFields.get(6).getText().toUpperCase());
			...
			int dialogueResult = JOptionPane.showConfirmDialog(this, "Are you sure you want to Save this Product?", "Warning!", JOptionPane.YES_NO_OPTION);
			if(dialogueResult==JOptionPane.YES_OPTION){
				selectedProduct.setId(txtFields.get(0).getText());
				selectedProduct.setDescription(txtFields.get(1).getText());
				...
				selectedProduct.setSupplier(txtFields.get(4).getText());
